...
public:
    ...
    //coord pos atual
    inline Coord getPos()const {

        return pos;
    }

    // retornar uma coord da pos ant
    inline Coord getAnt()const {

        return ant;
    }

    //retorna o custo até o noh
    ...
    //alterar valor
    /// alterar o valor até o noh
    inline void setG(const double& G){
        g = G;
    }

    ///altera o valor futuro
    inline void setH(const double& H){
        h = H;
    }

    ///muda pos atual com um coord
    inline void setPos(const Coord& C){
        pos = C;
    }

    ///muda a posição anterior do noh
    inline void setAnt(const Noh& N){
        ant = N.getPos();
    }
    inline void setAnt(const Coord& C){
        ant = C;
    }

    //Operadores
    /// if == para noh e coord
    inline bool operator==(const Noh& N) const{
...
//////////////////// CLASSE PARA ACHAR MAIOR //////////////////////
class CpAM
{
private:
    double custo;
public:
    inline CpAM(const Noh& N):custo(N.getCusto()){}
    inline bool operator()(const Noh& N)const{

        return (custo < N.getCusto());
    }
};
/// FIM GABRIEL
